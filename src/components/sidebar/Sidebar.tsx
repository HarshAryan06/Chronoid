...
const SCENIC_PLACEHOLDER =
  'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=120&auto=format&fit=crop&q=80';

export const Sidebar: React.FC<SidebarProps> = ({ config, setConfig, onPreviewFilter, imageSrc }) => {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (
        activePicker === 'frame' &&
        framePickerRef.current &&
        !framePickerRef.current.contains(event.target as Node)
      ) {
        setActivePicker(null);
      }
    }
  ...
  const handleChange = (key: keyof PolaroidConfig, value: string | number | boolean) => {
    if (key === 'frameColor') {
      const optimalTextColor = getOptimalTextColor(value as string);
      setConfig((prev) => ({
        ...prev,
        [key]: value,
    ...
    if (key === 'filter' && onPreviewFilter) {
      onPreviewFilter(value as string);
    }
  };
      ...
      <div className="w-full max-w-[340px] p-6 md:p-8 flex flex-col gap-8 flex-1 md:overflow-y-auto pb-24 md:pb-20">
        ...
        <div className="space-y-3 relative" ref={calendarWrapperRef}>
          ...
          >
            ...
            <div className="absolute right-0 top-0 bottom-0 w-12 flex items-center justify-center">
              <CalendarIcon
                className="w-5 h-5 text-stone-500 dark:text-stone-400 group-hover:text-stone-800 dark:group-hover:text-stone-100 transition-colors"
                strokeWidth={2.5}
              />
            </div>
          </div>
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-full border-2 border-stone-800 dark:border-stone-200 transition-all hover:scale-110 active:scale-90 ${
                    config.textColor.toLowerCase() === color.toLowerCase()
                      ? 'ring-2 ring-stone-800 dark:ring-white ring-offset-2 dark:ring-offset-[#1E1E1E]'
                      : ''
                  }`}
              ...
              <div className="relative" ref={textPickerRef}>
                ...
                {activePicker === 'text' && (
                  <div className="absolute z-50 right-0 top-full mt-3">
                    <ColorPicker color={config.textColor} onChange={(c) => handleChange('textColor', c)} />
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative mb-3" ref={fontDropdownRef}>
            ...
            >
              <span
                className="truncate text-stone-800 dark:text-stone-100 text-base font-bold"
          ...
          <div className="flex gap-3">
            {TEXT_STYLE_OPTIONS.map((btn) => (
              <button
                key={btn.key}
                onClick={() =>
                  handleChange(btn.key, !config[btn.key as keyof PolaroidConfig])
                }
          ...
          <div className={`${containerClasses} p-4 transition-colors`}>
            <div className="grid grid-cols-5 gap-4 justify-items-center">
              {FRAME_COLORS.map((color) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full border-2 border-stone-800 dark:border-stone-200 transition-all hover:scale-110 ${
                    config.frameColor.toLowerCase() === color.toLowerCase()
                      ? 'ring-2 ring-stone-800 dark:ring-white ring-offset-2 dark:ring-offset-[#1E1E1E]'
                      : ''
                  }`}
                  style={{ backgroundColor: color }}
              ...
              <div className="relative" ref={framePickerRef}>
                ...
                {activePicker === 'frame' && (
                  <div className="absolute z-50 right-0 top-full mt-3">
                    <ColorPicker color={config.frameColor} onChange={(c) => handleChange('frameColor', c)} />
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className={`${containerClasses} p-4 space-y-5 transition-colors`}>
            <div className="flex gap-2">
              {CORNER_RADIUS_PRESETS.map(({ value, label }) => {
                const isActive = config.cornerRadius === value;
                return (
                  <button
                    key={value}
                    onClick={() => handleChange('cornerRadius', value)}
            ...
            <div className="flex items-center gap-4 h-6">
              ...
              />
              <span className="text-[10px] font-bold text-stone-500 dark:text-stone-400 w-6 text-right font-mono">
                {config.cornerRadius}
              </span>
            </div>
          </div>
                  ...
                  >
                    ...
                    >
                      <div
                        ...
                        style={{
                          filter: filter.value,
                          backgroundImage: `url("${imageSrc || SCENIC_PLACEHOLDER}")`,
                        }}
                      ></div>
                    </div>
                    <span
                      className={`text-[9px] font-bold uppercase tracking-tight text-center max-w-[48px] truncate transition-colors ${
                        isActive ? 'text-stone-800 dark:text-white' : 'text-stone-500 dark:text-stone-400'
                      }`}
                    >
                      {filter.name}
                    </span>
